...
# These four cases are the full 2x2 matrix from the issue (issue
# open/closed x close-came-from-an-implementation-merge or not),
# demonstrated for the previously-exempted role, now denied in every
# quadrant where the issue is not open -- including the legitimate
# merge-close shape that used to be allowed.
...
# open-issue control (the other half of the 2x2 matrix): an observer role
# with a real Approve on an OPEN issue is unaffected by any of this --
# the removed exemption only ever touched the closed-issue precondition.
# (mode "human" is a PR-review APPROVED, not role-text-matched, so it
# authorizes any role/branch pair the same way.)
