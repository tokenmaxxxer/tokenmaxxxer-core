...
- **DEPENDS-ON (narrow, per role).** What a role's own conclusion is allowed
  ...
  - ux-design depends on `hypothesis` and `product-record` (the accepted
    ...
    and carries a WAKES-ON edge (routed per `docs/specs/wake-routing.md`) —
    and does not dictate what counts as good design; that judgment is
    ux-design's own.
  - verify depends on `coding-record`, `qa-record`, and `review-record` — it
    ...
    WAKES-ON edges (routed per `docs/specs/wake-routing.md`), and emits its
  ...
  - reflect depends on the subject's other role records (`coding-record`,
    ...
    carries its own WAKES-ON edge (routed per `docs/specs/wake-routing.md`),
    and may emit `finding` back-edges at `severity: advisory` — and does
    not dictate what the retro concludes; that judgment is reflect's own.
...
- Findings addressed to a role are part of that role's WAKES-ON triggers,
  routed per `docs/specs/wake-routing.md` — this contract only defines the
  `finding` record shape, not who wakes to answer it.
