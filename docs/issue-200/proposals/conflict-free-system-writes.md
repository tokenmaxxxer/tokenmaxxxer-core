...
**Report path — chosen:** derive the report path from the proposal's own path structure, not a separately-tracked "issue number" variable — when the proposal lives at `docs/issue-<n>/proposals/...` (contract v3 s19's own layout, what this repo and every role session actually use), the hunt report goes to `docs/issue-<n>/reports/hunt-<slug>.md`. `warrant/hooks/directive.sh`'s own generic (non-contract) protocol at lines 27/53 describes a repo with no issue number at all (`docs/proposals/YYYY-MM-DD-<slug>.md`); for that shape the report path is left unchanged (`docs/reports/hunt-<date>-<slug>.md`, still date+slug-keyed) — this issue's acceptance criteria only tests the contract v3 shape this repo runs under, and inventing an issue number where the proposal's own path carries none would be undocumented, not a fix. `warrant/hooks/directive.sh` line 76 and `warrant/agents/warrant-hunter.md` are both updated to state the derivation rule (read the scoping segment from the proposal's own path) rather than asserting a bare `docs/issue-<n>/reports/...` template as if every repo has one.
...
3. **`warrant/agents/warrant-hunter.md`**: update the report-path line (81) and the two example lines (150, 153) to state the derivation rule: when the proposal path is `docs/issue-<n>/proposals/...`, the report goes to `docs/issue-<n>/reports/hunt-<proposal-slug>.md`; otherwise (no issue segment in the proposal path) it stays `docs/reports/<date>-hunt-<proposal-slug>.md`, unchanged.
4. **`warrant/hooks/directive.sh`**: update lines 27/53 (the generic proposal-path template) to note the contract v3 issue-scoped variant exists alongside the flat one, and update line 76 (the report-path text handed to the hunter) to state the same derivation rule as item 3, so the dispatcher and the agent file agree.
