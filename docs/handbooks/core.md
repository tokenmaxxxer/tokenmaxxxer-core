...
## `record-shape-gate.sh` extension — parameterized record-section-shape-family gate (issue #263)

Extends the existing `core/hooks/record-shape-gate.sh` (previously
hardcoded to the `implementation` role's own phase-2 record shape,
issue #52 — that check runs first, unchanged) with a config-driven
`CHECKERS` dispatch that folds the 145 `record-section-shape`-family
hooks across all 43 rulebooks into `core/hooks/record-shape-config.json`.
Promote-first: none of the 145 source hooks or their `hooks.json`
entries were removed or modified.

**Dispatch.** After the hardcoded `implementation`-role check runs (and
does not already claim the write), reads `CLAUDE_ROLE` and looks up that
key in the config file (one entry per rulebook, 145 rows total); a role
with no entry, or a missing/unreadable config file, passes through
silently (exit 0). Each row's `target_path_regex` is matched against the
reconstructed write's normalized path, then dispatched to its
`check_type` handler. A Bash-tool write matching a row is a silent
pass-through (the gate cannot reconstruct a Bash-written file's content).

**`check_type` vocabulary**, generated by
`scripts/extract-record-shape-config.py` (a mechanical extractor —
regexes each of the 145 source hooks for its own shape/path/kill-switch
literals, since a full hand-read of 145 files was out of budget; every
row carries a `confidence: high|low` column, 86 high / 59 low at
delivery — per-hook detail and the extractor's rationale live in
`docs/issue-263/reports/implementation/survey.md` and
`docs/issue-263/reports/implementation.md`):
`checklist_entry_fields` (a marker + N required per-entry fields must
all appear in the write), `section_markers_conditional` (N required
section/field literals must all appear — this check_type also absorbs
the record-level required-frontmatter-field variant the survey's
4-shape sample did not name), `field_literal_token_cooccurrence` (at
least one of a row's required literal tokens must appear),
`methodology_checklist_gated` (a topic-keyword trigger; the fixed
methodology field-set enumeration itself is deferred to a future
