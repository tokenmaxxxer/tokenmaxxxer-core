...
SCOUT BRIEF, then build immediately: if scouting ran, writing docs/issue-<n>/reports/<role>/scout-brief.md is MANDATORY — a scout pass that produces no file is treated as not having scouted, full stop, no matter how much searching happened. Compress the brief content into at most 10 lines — category must-bes, chosen performance axes, adopt/skip patterns, one line on segment fit, and which stage count / which mode (parallel or batched-sequential fallback) the pass actually used. The brief feeds the build direction and any worker contracts directly. It is a steering input, not a report deliverable: no battlecards, no SWOT, no competitor matrix.

SOURCE LINKS (mandatory, exempt from the 10-line cap): every claim in the brief that came from the web — exemplar names, must-bes, praised/complained patterns — carries its source. Add a `Sources:` list of the URLs actually consulted, appended after the 10 content lines and not counted against them. No source, no claim: an assertion with no traceable source is stated as an assumption (labeled as such), never as a finding.
